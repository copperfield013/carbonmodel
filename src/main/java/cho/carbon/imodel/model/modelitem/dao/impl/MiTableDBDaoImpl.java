...
public class MiTableDBDaoImpl implements MiTableDBDao {
	...
	public List queryNewAddCol(String dataBaseName) {
		...
		.append(" WHEN '107' THEN	'timestamp(3)' ")
		.append(" WHEN '108' THEN	'datetime(3)' ")
		.append(" 	END	) ")
		.append(" FROM")
	...
	public List queryEditCol(String dataBaseName) {
		...
		.append(" 	CONCAT( 'alter table ', c.belong_table, ' CHANGE COLUMN ', c.code, ' ', c.code, ' ', c.mType ) ")
		...
		.append(" WHEN '106' THEN	'date' ")
		.append(" WHEN '107' THEN	'timestamp(3)'")
		.append(" WHEN '108' THEN	'datetime(3)' 		END")
	...
	public List queryCreRelaTabFun(String dataBaseName) {
		...
		.append(" FROM	t_cc_model_item WHERE 	type in('1', '101', '102', '103') 	) a")
	...
	public List queryCreRelaTab(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_r1' ) tablename FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
	...
	public List queryCreEntityFileTbaF1(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_f1' ) tablename, code FROM t_cc_model_item WHERE type in('1', '101', '102', '103') ) a")
	...
	public List queryCreEntityFileTbaF2(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_f2' ) tablename, code FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
	...
	public List queryCreEntityFileTbaF3(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_f3' ) tablename, code FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
	...
	public List queryCreEntityTabH1(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_h1' ) tablename, code FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
	...
	public List queryCreEntityTabH2(String dataBaseName) {
		...
		.append(" \"PRIMARY KEY (`\",	a.code,	\"_HP`) USING BTREE) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC\" 	) ")
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_h2' ) tablename, code FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name  ")
		.append(" WHERE	b.table_name IS NULL ");
	...
	public List queryCreEntityTabD1(String dataBaseName) {
		...
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_d1' ) tablename, code FROM t_cc_model_item WHERE type in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
	...
	public List queryCreEntityTabc1(String dataBaseName) {
		...
		.append(" \"PRIMARY KEY (`ABP0001`) USING BTREE) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC\" 	) ")
		.append(" FROM")
		.append(" ( SELECT concat( 't_', code, '_c1' ) tablename, code FROM t_cc_model_item WHERE type in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
	...
	public List queryCreEntityTabc2(String dataBaseName) {
		...
		.append(" \"PRIMARY KEY (`ABP0001`) USING BTREE) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC\" 	) ")
		.append(" FROM")
		.append(" 	( SELECT concat( 't_', code, '_c2' ) tablename, code FROM t_cc_model_item WHERE type  in('1', '101', '102', '103') ) a")
		.append(" LEFT JOIN ( SELECT table_name FROM information_schema.TABLES t WHERE t.table_schema = '"+dataBaseName+"' ) b ON a.tablename = b.table_name ")
		.append(" WHERE	b.table_name IS NULL");
