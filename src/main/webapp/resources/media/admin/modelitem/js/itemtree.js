...
seajs.use(['dialog','utils', 'ajax', '$CPF'], function(Dialog, Utils, Ajax, $CPF){
    ...
    function initModelItemChild(pmiCode) {
    	...
    	Ajax.ajax('admin/modelItem/getModelItemStairChild', {
			...
			if (data.code == "400") {
				...
				var $parent = $(".collapse-header[data-code='"+pmiCode+"']", $page).next(".collapse-content")[0];	
				for ( var key in modelItemList) {
					...
					if (modelItem.type == 5) {//单行属性分组
						...
						//级联枚举属性
						initCascadeEnumAttr(modelItem, $parent);
    ...
    //单行组初始化方法
    function initGroup(modelItem, $parent) {
    	var attrGroupHtml = getGroupNode(modelItem);
     ...
     * 单行组
      */
    function addGroup(el, modelItem) {// 这个有用
    ...
    //单行组
    function getGroupNode(modelItem) {
    ...
    //多行组初始化方法
    function initMoreGroup(modelItem, $parent) {
    	var attrGroupHtml = getGroupMoreNode(modelItem);
	    var $html = $(attrGroupHtml).prependTo($($parent));
	   /* $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();*/
        drag($(".dragEdit-wrap").length);
    }
     ...
     * 多行组
      */
    function addMoreGroup(el, modelItem) {// 这个有用
        var $content = $(el).closest(".collapse-header").siblings(".collapse-content");
        var attrGroupHtml = getGroupMoreNode(modelItem);
        var $html = $(attrGroupHtml).prependTo($content);  
	    $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();
    ...
    //多行组
    function getGroupMoreNode(modelItem) {
    	var dragWrapLen = $(".dragEdit-wrap").length + 1 ;
        var attrGroupHtml = "<li class='attr-group'>" +
            "<div class='attr-group-title collapse-header'  data-code='"+modelItem.code+"'  data-type='"+modelItem.type+"' data-pCode='"+modelItem.parent+"'>" +
            ...
            "<span id='spanName' class='span-title'>"+modelItem.name+"</span>";
	         attrGroupHtml += "<div class='btn-wrap'>" +
	        /* "<i class='glyphicon glyphicon-pencil'></i>"+*/
	         "<i class='fa fa-pagelines confTwoLeverAttr'></i>"+
	         "<i class='fa fa-edit icon-edit'></i>"+
            "<i class='icon icon-add-sm group'></i>" +
     ...
     * 添加通用属性方法
      */
    function addCommAttr(el, modelItem) {// 这个有用
        var $content = $(el).closest(".collapse-header").siblings(".collapse-content");
        var attrGroupHtml = getCommAttrNode(modelItem);
        var $html = $(attrGroupHtml).prependTo($content);  
	    $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();
	    addUnfold(el)
        drag($(".dragEdit-wrap").length);
    ...
    //通用属性初始化方法
    function initCommAttr(modelItem, $parent) {
    	var attrGroupHtml = getCommAttrNode(modelItem);
	    var $html = $(attrGroupHtml).prependTo($($parent));
	   /* $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();*/
        drag($(".dragEdit-wrap").length);
    ...
    //通用属性节点
    function getCommAttrNode(modelItem) {
    	var dragWrapLen = $(".dragEdit-wrap").length + 1 ;
     ...
     * 添加级联枚举属性方法
      */
    function addCascadeEnumAttr(el, modelItem) {// 这个有用
        var $content = $(el).closest(".collapse-header").siblings(".collapse-content");
        var attrGroupHtml = getCascadeEnumAttrNode(modelItem);
        var $html = $(attrGroupHtml).prependTo($content);  
	    $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();
	    addUnfold(el)
        drag($(".dragEdit-wrap").length);
    ...
    //级联枚举初始化方法
    function initCascadeEnumAttr(modelItem, $parent) {
    	var attrGroupHtml = getCascadeEnumAttrNode(modelItem);
	    var $html = $(attrGroupHtml).prependTo($($parent));
	   /* $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();*/
        drag($(".dragEdit-wrap").length);
    ...
    //级联枚举
    function getCascadeEnumAttrNode(modelItem) {
    	var dragWrapLen = $(".dragEdit-wrap").length + 1 ;
        var attrGroupHtml = "<li class='add-attr clear-fix'>" +
            "<div class='attr-group-title collapse-header'  data-code='"+modelItem.code+"'  data-type='"+modelItem.type+"' data-pCode='"+modelItem.parent+"'>" +
            "<div class='icon-label attr'>" +
            ...
            "<span id='spanName' class='span-title'>"+modelItem.name+"</span>";
	         attrGroupHtml += "<div class='btn-wrap'>" +
	         "<i class='fa fa-pagelines confCascadeEnumChild'></i>"+
            "<i class='fa fa-edit icon-edit'></i>"+
            "<i class='glyphicon glyphicon-trash delModelItem'></i>" +
            "</div>" +
            "</div>" +
     ...
     * 添加级联引用属性方法
      */
    function addCascadeReferenceAttr(el, modelItem) {// 这个有用
        var $content = $(el).closest(".collapse-header").siblings(".collapse-content");
        var attrGroupHtml = getCascadeReferenceAttrNode(modelItem);
        var $html = $(attrGroupHtml).prependTo($content);  
	    $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();
	    addUnfold(el)
        drag($(".dragEdit-wrap").length);
    ...
    //级联引用初始化方法
    function initCascadeReferenceAttr(modelItem, $parent) {
    	var attrGroupHtml = getCascadeReferenceAttrNode(modelItem);
	    var $html = $(attrGroupHtml).prependTo($($parent));
	   /* $html.find("select").css({"width":"7%","marginLeft":"2"}).select2();*/
        drag($(".dragEdit-wrap").length);
    ...
    //级联引用
    function getCascadeReferenceAttrNode(modelItem) {
    	var dragWrapLen = $(".dragEdit-wrap").length + 1 ;
        var attrGroupHtml = "<li class='add-attr clear-fix'>" +
            "<div class='attr-group-title collapse-header'  data-code='"+modelItem.code+"'  data-type='"+modelItem.type+"' data-pCode='"+modelItem.parent+"'>" +
            "<div class='icon-label attr'>" +
          ...
          /*  "<i class='icon icon-save icon-edit'></i>" +*/
           /* "<i class='icon icon-add-sm group'></i>" +*/
	         "<i class='fa fa-pagelines confCascadeReferenceChild'></i>"+
            "<i class='fa fa-edit icon-edit'></i>"+
            "<i class='glyphicon glyphicon-trash delModelItem'></i>" +
           /* "<i class='icon delModelItem'></i>" +*/
            /*"<i class='icon icon-arrow-sm active'></i>" +*/
    ...
    //级联引用孩子添加
    $("#modelItemEdit").on("click", ".confCascadeReferenceChild", function() {        
        var $labelBar = $(this).closest(".label-bar");
        
        var modelItemCode = $labelBar.closest(".collapse-header").attr("data-code");
        Dialog.openDialog("admin/modelItem/getCasRefChild?pmiCode=" + modelItemCode, "级联引用孩子", "", {
            width :1000,
            height : 500
        });
    ...
    //二级属性孩子树
    $("#modelItemEdit").on("click", ".confTwoLeverAttr", function() {        
    	
        var $labelBar = $(this).closest(".label-bar");
        var modelItemCode = $labelBar.closest(".collapse-header").attr("data-code");
        
        // 这里要弹出标签页
        Dialog.openDialog("admin/modelItem/addTwoLevelMaping?moreLineCode=" + modelItemCode, "二级属性组", "", {
            width :1000,
            height : 500
        });
	...
	function addEntityOPT() {
		var $select = $("#modelItemEdit .entity-title").find(".node-ops-type");	
...
//	        	judgeSave();
  ...
  //普通 引用->引用属性初始化方法
    ...
    function initTag(subdomain,abcattrCode, id,name,opt,order,parent,isRelative) {  
    	...
    	var tagHtml = "<li class='add-tag clear-fix'>" +
	        ...
	        tagHtml += "<i class='icon delModelItem'></i>" +
	        "</div>" +
	        "</div>" +
	        "</li>"
	...
	function getEntity(entity) {		
		...
		$("#modelItemEdit .entity-title>.edit-input").val(cnName);
		$("#modelItemEdit .entity-title>.entity-only-title").html(cnName);
		$("#modelItemEdit .entity-edit-wrap").addClass("active");
	}
    ...
    function popsm(el) {
        ...
        if (addTagLength > 0 || isMoreAttr ) {
            ...
            html += "<li class='card-list add_VALUE_ITEM' modelItemType='2' modelItemShowName='添加普通属性'>" +
                "<i class='icon icon-card-attr'></i>" +
                "<span class='text'>添加普通属性</span>" +
                "</li>" +
                "<li class='card-list add_ENUM_ITEM' modelItemType='201' modelItemShowName='添加枚举属性'>" +
                "<i class='icon icon-card-attr'></i>" +
                "<span class='text'>添加枚举属性</span>" +
                "</li>" +
                ...
                "<span class='text'>添加引用属性</span>" +
                "</li>" +
                "<li class='card-list add_CASCADE_ENUM_ITEM' modelItemType='206' modelItemShowName='添加级联枚举属性'>" +
                "<i class='icon icon-card-attr'></i>" +
                "<span class='text'>添加级联枚举属性</span>" +
                "</li>" +
                "<li class='card-list add_CASCADE_REFERENCE_ITEM' modelItemType='207' modelItemShowName='添加级联引用属性'>" +
                "<i class='icon icon-card-attr'></i>" +
    ...
    */
    ...
    $("#modelItemEdit").on("click", ".tag-search", function (e) {
        e.stopPropagation();        
    });
    
    //搜索标签点击事件绑定
    $("#modelItemEdit").on("input propertychange", ".tag-search", function (e) {
        e.stopPropagation();               
  ...
  //引用->引用属性保存修改方法
   ...
   /* *//**
    ...
    //提醒有未保存的节点
    function judgeSave() {    	
        /*var editBar = $("#modelItemEdit").find(".label-bar.edit");
        var editEntity = $("#modelItemEdit").find(".entity-edit-wrap.edit");
  ...
  //rFilterSave  保存修改方法
    ...
    //收缩事件绑定
    $("#modelItemEdit").on("click", ".icon-arrow, .icon-arrow-sm", function (e) {
    	var $labelBar = $(this).closest(".label-bar");
    	var pmiCode = $labelBar.closest(".collapse-header").attr("data-code");
    ...
    //跟实体添加事件绑定
    $("#modelItemEdit").on("click", ".icon-add, .icon-add-abc", function (e) {
        e.stopPropagation();
    ...
    //属性组 多值属性 添加事件绑定
    $("#modelItemEdit").on("click", ".icon-add-sm", function (e) {
        e.stopPropagation();
        var hasSave = judgeSave();
        if(hasSave){
            return;
    ...
    //标签添加
    $("#modelItemEdit").on("click", ".icon-add-tag", function (e) {
        e.stopPropagation();
    ...
    //关系下标签添加
    $("#modelItemEdit").on("click", ".icon-add-tag-relative", function (e) {
        e.stopPropagation();
        $(this).closest(".label-bar.tag").addClass("edit");
        removePop();        
        popRelativeTag(this);
    ...
    //filterGroup 事件绑定
    $("#modelItemEdit").on("click", ".icon-add-filterGroup", function (e) {
  ...
  //filter 事件绑定
    $("#modelItemEdit").on("click", ".icon-add-filter", function (e) {
    ...
    //删除属性事件绑定
    $("#modelItemEdit").on("click", ".icon-trash , .delModelItem", function (e) {
        e.stopPropagation();
        removePop();
    ...
    //添加页中的事件绑定
    //这个有用
    $("#modelItemEdit").on("click", ".card>li.card-list", function (e) {
        e.stopPropagation();
        if ($("#modelItemEdit").find(".icon-add.active").length > 0) {
            var el = $("#modelItemEdit").find(".icon-add.active")[0];
        } else if ($("#modelItemEdit").find(".icon-add-sm.active").length > 0) {
            var el = $("#modelItemEdit").find(".icon-add-sm.active")[0];
        } else if ($("#modelItemEdit").find(".icon-add-abc.active").length > 0) {
            var el = $("#modelItemEdit").find(".icon-add-abc.active")[0];
        }else if ($("#modelItemEdit").find(".icon-add-filterGroup.active").length > 0) {
            var el = $("#modelItemEdit").find(".icon-add-filterGroup.active")[0];
        }else if ($("#modelItemEdit").find(".icon-add-filter.active").length > 0) {
        ...
        if ($(this).hasClass("add-tag")) {
            //addTag(el);
        } else if ($(this).hasClass("add_VALUE_ITEM")) {// 这个有用
        	 var mipCode = $(el).closest(".collapse-header").attr("data-code");
         	//这里要弹出框， 保存并回显
         	/* Dialog.openDialog("admin/modelItem/addAttr?modelItemType=2&mipCode=" +mipCode, "添加普通属性", "", {
                  width :920,
                  height : 400,
                  events		: {
      				afterSave	: function(modelItem){
           ...
           // addRelative(el);
        } else if ($(this).hasClass("add_ENUM_ITEM")) {//这个有用
        	
        	var mipCode = $(el).closest(".collapse-header").attr("data-code");
         	//这里要弹出框， 保存并回显
         	 /*Dialog.openDialog("admin/modelItem/addAttr?modelItemType=201&mipCode=" +mipCode, "添加枚举属性", "", {
                  width :920,
                  height : 400,
                  events		: {
      				afterSave	: function(modelItem){
    ...
    //弹出页中的事件绑定添加标签
    $("#modelItemEdit").on("click", ".tag-checkbox-input", function (e) {   
        e.stopPropagation();
        var el = $("#modelItemEdit").find(".icon-add-tag.active");
        if(el.length == 0) {
        	el = $("#modelItemEdit").find(".icon-add-tag-relative.active")[0];
        }else {
        	el = $("#modelItemEdit").find(".icon-add-tag.active");
        }   
        var text = $(this).val();
    ...
    //标签向左移动事件绑定
    $("#modelItemEdit").on("click", ".icon-toleft", function () {
        ...
        $("#modelItemEdit").find(".icon-toright").removeClass("ban");
    ...
    //标签向右移动事件绑定
    $("#modelItemEdit").on("click", ".icon-toright", function () {
        if ($(this).hasClass("ban")) {
            return;
        }
        var $ul = $(this).prev(".tag-content").children("ul");
        var marginLeft = parseFloat($ul.css("marginLeft"));
        $("#modelItemEdit").find(".icon-toleft").removeClass("ban");
        if (marginLeft > -80) {
            marginLeft = 0;
            $(this).addClass("ban");
        } else {
    ...
    //标签删除图标点击事件绑定
    $("#modelItemEdit").on("click", ".tag-content .icon-delete", function(){
    ...
    //双击编辑
    $("#modelItemEdit").on("dblclick", ".label-bar", function(){
    ...
    //双击编辑
    $("#modelItemEdit").on("dblclick", ".entity-title", function(){ 
    	$(this).find(".edit-input").removeAttr("disabled");
    	$(this).find("select").removeAttr("disabled");
        $(this).addClass("edit");
    })
     ...
     //保存
    $("#modelItemEdit").on("click", ".icon-edit", function() {        
        var $labelBar = $(this).closest(".label-bar");
        var modelItemCode = $labelBar.closest(".collapse-header").attr("data-code");
        var modelItemType = $labelBar.closest(".collapse-header").attr("data-type");
        var mipCode = $labelBar.closest(".collapse-header").attr("data-pcode");
        
        //这里要弹出框， 保存并回显
        Dialog.openDialog("admin/modelItem/addAttr?modelItemCode="+modelItemCode+"&modelItemType="+modelItemType+"&mipCode=" +mipCode, "编辑属性" +modelItemCode, "", {
            width :920,
            height : 400,
            events		: {
				afterSave	: function(modelItem){
    ...
    //更改节点span  显示的值
    function updateNode(modelItem, $labelBar) {
    	$labelBar.find("#spanName").html(modelItem.name);
    }
    
    //删除-全部
    $("#modelItemEdit").on("click", ".opera.confirm", function(e) {  
    	e.stopPropagation();    
    ...
    //删除-仅组
    $("#modelItemEdit").on("click", ".opera.only-group", function(e) { 
    	e.stopPropagation();
    	var el = $(".delModelItem.active")[0];       
        var labelBar = $(".delModelItem.active").closest(".label-bar");        
    ...
    //实体选择点击事件绑定
    $("#modelItemEdit").on("click", ".entity_attr", function() {
    ...
    //修改名称
    $("#modelItemEdit").on("change", "select.abc-attr", function(){
