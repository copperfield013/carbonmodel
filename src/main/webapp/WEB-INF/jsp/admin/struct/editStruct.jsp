		...
		<div class="row">
			<div class="col-lg-12">
				<form class="bv-form form-horizontal validate-form" action="admin/structBase/createStrucBase">
					<input type="hidden" name="strucBase.id" value="${strucBase.id }">
					<div class="form-group">
						<label class="col-lg-2 control-label" for="strucBase.title">结构体名称</label>
						<div class="col-lg-5">
							<input type="text" name="strucBase.title" value="${strucBase.title }" placeholder="名字不能为空" data-bv-notempty="true" data-bv-notempty-message="名字不能为空" class="form-control name" />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-5">
							<select name="strucBase.opt">
								<option value="1" <c:if test="${strucBase.opt eq '1'}">selected </c:if>>读</option>
								<option value="2" <c:if test="${strucBase.opt eq '2'}">selected </c:if>>写</option>
							</select>
						</div>
					</div>
